...
class Auth {
    public static function getBucketAuth ($bucket_name) {
        $bucketAuth = new stdClass();
        $bucketAuth->list = false;
        $bucketAuth->read = false;
        $bucketAuth->create = false;
        $bucketAuth->edit = false;
        $bucketAuth->delete = false;
    ...
    private static function checkBucketAuthByTypeExists ($bucket_name, $type) {
        ...
        $stmt = $db->prepare("SELECT COUNT(*) FROM Auth WHERE bucket_name = :bucket_name AND auth_type = :auth_type");
    ...
    public static function addBucketAuth ($bucket_name, $auth_type, $identifier, $secret, $auth_object) {
        $db = Database::getSingleton();

        $stmt = $db->prepare(
            "INSERT INTO Auth
                (bucket_name, auth_type, identifier, secret, can_list, can_read, can_create, can_edit, can_delete)
            VALUES
                (:bucket_name, :auth_type, :identifier, :secret, :can_list, :can_read, :can_create, :can_edit, :can_delete)");
    ...
    public static function removeBucketAuth ($bucket_name, $auth_type, $identifier = null, $secret = null) {
        ...
        if ($auth_type === "public") {
            $stmt = $db->prepare("DELETE FROM Auth WHERE bucket_name = :bucket_name AND auth_type = :auth_type");
            return $stmt->execute([ "bucket_name" => $bucket_name, "auth_type" => $auth_type ]);
        }

        if ($auth_type === "basic") {
            $stmt = $db->prepare("DELETE FROM Auth WHERE bucket_name = :bucket_name AND auth_type = :auth_type AND identifier = :identifier");
            return $stmt->execute([ "bucket_name" => $bucket_name, "auth_type" => $auth_type, "identifier" => $identifier ]);
        }

        if ($auth_type === "bearer") {
            $stmt = $db->prepare("DELETE FROM Auth WHERE bucket_name = :bucket_name AND auth_type = :auth_type AND secret = :secret");
            return $stmt->execute([ "bucket_name" => $bucket_name, "auth_type" => $auth_type, "secret" => $secret ]);
        }
